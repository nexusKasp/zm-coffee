      ...
      form.addEventListener('submit', function(e) {
        ...
        inputs.forEach(input => {
          if (!input.value.trim()) {
            ...
            formGroup.querySelector('.form-error').textContent = 'Это поле обязательно';
          } else {
            const formGroup = input.closest('.form-group');
            ...
            // Phone validation
            if (input.id === 'reg-phone' && !validatePhone(input.value)) {
              isValid = false;
              formGroup.classList.add('error');
            ...
            if (input.id === 'reg-confirm' && input.value !== document.getElementById('reg-password').value) {
              isValid = false;
              formGroup.classList.add('error');
